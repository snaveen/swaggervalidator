...
public class SimpleRouteMatcher {
    ...
    public void addRoute(String httpMethod, String url, String acceptedType) {
        HttpMethod method;
        try {
            method = HttpMethod.valueOf(httpMethod);
        } catch (IllegalArgumentException e) {
            LOG.error("an invalid HTTP method part: "
                              + httpMethod
                              + ".");
            return;
        }
        addRoute( method, url, acceptedType, new Object());
    }
    
    public List<RouteEntry> findTargetsForRequestedRoute(String httpMethod, String path) {
        HttpMethod method;
        try {
            method = HttpMethod.valueOf(httpMethod);
        } catch (IllegalArgumentException e) {
        ...
        return findTargetsForRequestedRoute( method, path);
    }
    
    public RouteEntry findTargetsForRequestedRoute(String httpMethod, String path, String acceptType) {
        RouteEntry routeEntry = null;
        List<RouteEntry> routeMatches = findTargetsForRequestedRoute(httpMethod, path);
        if (acceptType != null)
            routeEntry = findTargetWithGivenAcceptType(routeMatches, acceptType);
        else if (routeMatches.size()>0) {
            routeEntry=routeMatches.get(0);
            if(routeMatches.size()>1) {
                LOG.warn("Multiple matches found going with first match");
            }
        }
